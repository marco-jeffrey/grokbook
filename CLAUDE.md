...
| `"data-on-click": at.post(...)` | `data.on("click", at.post(...))` — or raw: `"data-on:click"` |
| Reading form body with `parse_qs` | `await get_signals(c.req)` |
| `data-on-keydown` (hyphen separator) | `data-on:keydown` (colon separator) — **ALL Datastar event attrs use `data-on:` with colon, NEVER hyphen** |
